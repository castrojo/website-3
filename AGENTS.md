...
npm run typecheck          # TypeScript validation (vue-tsc)
npm run test:run           # Vitest suite (leaves public/dakota-versions.json dirty; discard)
```
...
- **Wolves theater typography:** The intro is a 10-foot theater experience. Keep every cue theater-readable; standard type is the minimum and dominant cues must remain visibly larger and heavier. Fit copy with authored line breaks, timing, or cue splits, never smaller type.
