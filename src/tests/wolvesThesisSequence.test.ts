...
describe('wolves thesis sequence', () => {
  it('preserves the approved thesis timing and final call to action', () => {
    ...
    expect(getWolvesThesisState(348)).toMatchObject({ active: true, mode: 'welcome', text: 'Welcome to the path.', dayPulse: true, hudLabel: 'Incoming Signal: Universal Blue' })
    expect(getWolvesThesisState(350.499).mode).toBe('corruption')
