...
- A guardian's bonded dinosaur renders as a **companion nameplate** — a separate chamfered card beside the guardian plate (`.wolves-companion-plate` in `WolvesIntroOverlay.vue`), labelled `GUARDIAN BOND`, with the artwork prominent and breaking out over the card's top edge (the card carries the clip-path; the art overlaps via negative bottom margin), plus the dinosaur's authored name (only if a lore character sheet names it — currently Karl and Alamo; Bob's and Kaslin's torosauruses are unnamed and show species only) and italic scientific name. Both plates share one anchored `.wolves-guardian-plate-row` flex wrapper, which now carries the position variants (`-left`/`-right`/`-raised`) and the swap transition classes. The bond lookup lives in `src/data/wolves-guardian-dinosaur-bonds.ts` — a small curated `guardianName -> { dinosaurSpeciesId, dinosaurName? }` list, intentionally *not* parsed from lore markdown at runtime (the intro overlay is a lightweight, performance-sensitive component and shouldn't depend on the full lore loader). Update this list whenever a new `guardian-bond` record is added for a guardian who also appears in the intro sequence, and never invent a dinosaur name — `dinosaurName` must come from an authored character sheet. Guardians with no documented bond render with no companion plate — this is expected, not a bug.
