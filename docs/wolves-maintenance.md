...
When the user clicks the primary "Start soundtrack" / "Start Experience" button (the first `idle`/`error` -> playback transition in `WolvesSoundtrack.vue`, or the parent-driven `v-model:playing` transition used by `WolvesApp.vue`'s "JOIN THE EVOLUTION" button), a fullscreen intro sequence plays before the live YouTube playlist experience begins. The current production sequence is a focused two-part flow followed by an immediate handoff into the live playlist:

1. `wolves-prologue` — a black-screen, text-only cold open (no video id) driven by a background-only YouTube audio embed (the Gayane Ballet Suite Adagio / "2001: A Space Odyssey" cue). Per-cue static or day/night-crossfade background artwork is still supported for the two specific authored beats (the Collapse image and the single KubeCon hero photo); all other beats remain plain black text. The prologue was reduced from the original full-length audio and has been authoritatively set to a 75s excerpt per the approved Task 1 request (2026-07-16). The user hand-edits `wolves-intro-sequence.ts` to choose the cue copy; cue `start`/`end` timestamps span exactly 75 seconds. Multi-line cue text remains authored with real `\n` characters (not HTML) and requires `white-space: pre-line` on `.wolves-intro-overlay-text`. The prologue duration must not be expanded beyond 75s without a fresh user request.
