...
1. `wolves-prologue` — a black-screen, text-only cold open (no video id) set to a background-only YouTube audio embed (the Gayane Ballet Suite Adagio / "2001: A Space Odyssey" cue). Per-cue static or day/night-crossfade background artwork is supported for two specific beats (the Collapse image, a single KubeCon hero photo); every other beat stays plain black text. **Cut from 326s (the song's full 5:26) down to a 45s excerpt per explicit user request (2026-07-15)** — the user hand-edited `wolves-intro-sequence.ts` directly to choose which lines survive and rewrote several (including two new multi-line beats: "Until the Birth of AI, / and Society Decided, that / Guardians were not only unnecessary, but a threat." and an extra blank line inserted into the Clarke-style "for the heart of any race..." quote). The surviving cues' `start`/`end` timestamps were then retimed proportionally (each cue's original share of the total duration, scaled to fit exactly 45s) to close the dead/black gaps left by the removed lines. Multi-line cue text requires `white-space: pre-line` on `.wolves-intro-overlay-text` (added the same day) since this component intentionally never uses `v-html` — the user's line breaks are authored as real `\n` characters via a TS template literal, not `<br>` markup. **Widened from 45s to 60s the same day, per a follow-up user request**, again by proportionally rescaling every cue's `start`/`end` (same methodology, new target duration). The same request asked for "the last quote" (the Clarke-style "for the heart of any race is destroyed, and its will to survive is utterly broken" line) to read better — it was split into two separate sequential single-clause cues ("for the heart of any race is destroyed," then "and its will to survive is utterly broken,"), removing the mid-cue blank-line hack, so each clause now gets its own dedicated beat like its sibling lines. Do not re-collapse these two cues back into one, and do not re-expand `duration` beyond 60 or restore the originally-removed lines without a fresh user request.
...
- **Christopher Blecker's cue is gilded gold** via the `leader: true` field (`wolves-guardian-plate-leader` CSS in `WolvesIntroOverlay.vue`), per explicit user request (2026-07-15) to signify leadership, pairing with his existing "First Among Equals" title line. His window was also extended from 83-90s to 83-96s (matching Natali Vlatko's own end) so his plate stays up longer — re-verified via frame capture that his green Strand arm is still visibly in frame through 94-96s, so this is also a footage-accuracy correction, not purely a stylistic hold. `leader` is reserved for him alone; do not apply it to other guardians without a fresh user request. His title line carries four segments ("First Among Equals — The North Star — Uncompromising Purity — Platinum Member"), joined and rendered the same way as the original two, so all four read with equal visual weight — per explicit user request (2026-07-15) for "even association." "Platinum Member" (renamed from "Platinum Sponsor") additionally carries the cue's `blingTitle: 'Platinum Member'` field, which wraps just that exact substring in `wolves-guardian-plate-bling` — a looping gold shimmer sweep plus a pulsing glow, distinct from the plain title text around it — per a same-day follow-up request for a "distinctive bling effect." `blingTitle` must match a substring of the cue's title exactly (case-sensitive) or it silently renders with no effect; reserved for this one segment.
...
A one-time, fullscreen bridge between Track 0 ("7 Days to the Wolves") and Track 1 ("Ghosts In The Mist") plays two creators' real YouTube Shorts **side by side**, ping-ponging which side is active: Cassidy Williams (`@cassidoo`) on the left, taking the first turn, and Lindsay Nikole (`@LindsayNikole`) on the right. It highlights both creators equally by design — never let one dominate the layout or turn order. Turn order was swapped (Cassidy now goes first, Lindsay second) per explicit user request (2026-07-15); the previous order had Lindsay first.

Architecture:

- `src/data/wolves-creator-shorts.ts`: the **content surface** — two independent typed, ordered arrays, `wolvesCreatorShortsLindsayNikole` and `wolvesCreatorShortsCassidyWilliams`, each of `{ videoId, title, creatorName, channelUrl, orientation }`. This is the only editable part: swap, add, or remove entries within either list. Cassidy's list must stay on the left/first-turn side per explicit instruction (2026-07-15). `orientation` is `'vertical'` for normal 9:16 Shorts and `'horizontal'` for the rare real 16:9 video (see below) — YouTube's own embedded player automatically letterboxes/pillarboxes to fit the fixed 9:16 slot, so no extra layout code is needed per orientation, but the field documents *why* a given entry is an exception.
- `src/components/wolves/WolvesCreatorShortsInterstitial.vue`: fullscreen, `<Teleport to="body">` player (same reason as the intro overlay — immersive-layout ancestors use `transform`, which would otherwise confine a `position: fixed` overlay to its own widget). Creates **exactly two persistent `YT.Player` instances** (left/Cassidy, right/Lindsay), never destroyed/recreated mid-sequence. The active side autoplays; the inactive side is silently pre-cued on its next video (`cueVideoById`) so a swap is an instant flash-cut (`playVideo`), not a fresh load. When one creator's list runs out, the other continues solo (`loadVideoById`, autoplay) until it also finishes, then the component emits `complete`. Advances on `onStateChange` `ENDED` for the active side; a `skip` button in the bottom control bar triggers the exact same advance logic manually. `onError` on either side never blocks the ping-pong. Each side shows its own title + creator-name text beneath the video (sourced from our data, never YouTube's own truncatable on-video title). The whole interstitial's full-screen backdrop, plus the bottom control bar, both show the current-month Bluefin wallpaper (reusing the `public/img/wallpapers/bluefin-{MM}-night.webp` asset set already used by `WolvesApp.vue`'s wallpaper crossfade), per explicit user request (2026-07-15) to match "the rest of the presentation" instead of a plain black backdrop; a dark scrim (`wolves-creator-shorts-backdrop-scrim`, matching the existing controls-overlay scrim color) sits between the wallpaper and the video slots/text so legibility is unaffected. The control bar's Pause/Resume and Skip buttons are styled to match `WolvesSoundtrack.vue`'s existing icon buttons. This component's markup/logic is locked; only the two video lists it's given are editable.
...
- `src/composables/useYoutubeIframeApi.ts`'s shared `YoutubePlayer` interface gained `loadVideoById`/`cueVideoById` (additive, optional) to support this preloading mechanism; both are standard YouTube IFrame API methods.
...
**Removed entries**: Lindsay Nikole's "Hammerheads are getting hammered" (`K1DJNw2zHlo`) and Cassidy Williams's "React is like paella" (`cIaNRGkZQdM`) were removed from the rotation per explicit user request (2026-07-15). Lindsay's list is now 7 entries (was 8); Cassidy's list is at that point 5 entries (was 6).
