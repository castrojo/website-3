...
- **Trustee plates render in burnished silver** via the `trustee: true` cue field (`wolves-guardian-plate-trustee` CSS in `WolvesIntroOverlay.vue`): silver horizons/crest/label/burst (`#d1d5db`/`#e5e7eb`) and the `TRUSTEE // GUARDIAN` label, distinct from both the default blue plate and the singular gold leader plate. Reserved for Universal Blue trustees — currently Bob Killen's intro cue; Jorge Castro's Ghosts In The Mist theater plate in `WolvesComicReader.vue` mirrors the same silver palette. Do not apply to other guardians without a fresh user request.
