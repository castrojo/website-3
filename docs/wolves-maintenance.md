...
2. `wolves-intro` — the official YouTube IFrame Player embed of Bungie's "Destiny 2: Into the Light Cinematic" trailer (default unvoiced `youtubeVideoId: 'BV3BZKbpBns'`, optional voiced override `alternateYoutubeVideoId: 'BKm0TPqeOjY'`) with timed dialogue captions from `src/data/wolves-destiny-captions.txt` and character overlay text introducing the six Guardian characters. The default Guardian lower-third plate spans the viewport; explicitly paired Guardian cues retain their side anchors. The shared intro widget exposes the exact `Ikora voice over` checkbox during this segment only, switching sources through object-form `loadVideoById({ videoId, startSeconds }) while preserving native time and pause state. Its intro chapter label is **UNIVERSAL BLUE BRIEFING**.
