...
- **Creator Shorts section** (`.wolves-creator-shorts-section` in `src/WolvesApp.vue`, rendering `src/components/wolves/WolvesCreatorShorts.vue`): a single alternating YouTube Shorts player placed right after the hero, before the QR code section. Plays the ordered list in `src/data/wolves-creator-shorts.ts` (real public Shorts video IDs, one per entry) and loops back to the start when the last one ends. **Open surface**: the video list in `wolves-creator-shorts.ts` (swap, add, or remove `{ videoId, title, creatorName, channelUrl }` entries). **Locked**: the strict alternation and the rule that Lindsay Nikole's entry always comes first; the component's markup/styles/player logic.
...
1. `wolves-prologue` — a black-screen, text-only cold open (no video id) set to a background-only YouTube audio embed (the Gayane Ballet Suite Adagio / "2001: A Space Odyssey" cue), timed to the song's full 326-second runtime. The opening "Gardener and Winnower" cue stays plain black (nothingness, before Earth exists in the story). Most other cues cycle through the monthly Bluefin wallpaper day/night pairs (`public/img/wallpapers/bluefin-NN-{day,night}.webp`, the same asset set `WolvesApp.vue` uses live) as a slow day-to-night crossfade; a few beats use dedicated artwork instead (the Collapse day/night pair, a single KubeCon hero photo). See "Prologue background and text cues" below for the full data model.
