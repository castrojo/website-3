...
- **Scheduling**: `src/data/wolves-narrative-timeline.ts` maps every Track 0 second to one record. The timeline is locked authored data with three hard anchors: `arthur-c-clarke-4` at 0:00, `lorem-pursuit-1` at 150 to 220 seconds, and `blue-universal-acquires-wayland-yutani` at 398 to 425 seconds. Order is strictly chronological and narrative-driven; quotes are interleaved between chats to fill in context while chats move the story forward. Never reorder or reshuffle.
