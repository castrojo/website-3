...
- `src/data/wolves-creator-shorts.ts`: the **content surface** — a typed, ordered array of `{ videoId, title, creatorName, channelUrl }`. This is the only editable part: swap, add, or remove entries here. Order must stay strictly alternating and Lindsay Nikole must stay first per explicit instruction.
