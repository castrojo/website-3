...
- The page has two states, toggled by `isImmersive` in `WolvesApp.vue`:
  ...
  - **Immersive theater**: fullscreen HUD experience entered through the hero CTA. Escape or the exit button leaves it. The intro-to-cinematic handoff is a 1.4-second dissolve (`INTRO_HANDOFF_FADE_MS` in `src/WolvesApp.vue`).
...
- **Track 0 nameplate communications** (`.wc-stage-nameplate`): the large label text comes from `src/data/wolves-incoming-signal.txt` (see Open Surfaces) and crossfades in 200ms. The plate (`Nameplate.vue`, shared by the intro and stage) carries a Bluefin-branded emblem on its left: a small gold-ringed roundel that opens on the Bluefin raptor avatar and crossfades through the full `dinosaurSpecies` roster every 20 seconds (decorative, `aria-hidden`). The default HUD label is `Celebrating Five Years of Universal Blue`. Only the first line (currently `Welcome to Indie Cloud Native`) holds from load through 175.96 seconds — the remaining ambient signals name heroes (Hikari, M2, Mechaphippy) and would spoil the story if shown before their slides. The label holds `The Blue Delivers` for the locked Bluefin group through 196.36 seconds, then the remaining pre-delimiter ambient signals compress evenly into 196.36 to 229 seconds (right after the heroes they reference, ending on the pod line), followed by the split ImagePullBackOff and human-fallback lines through 345 seconds. A `---` line in the source splits those ambient signals from the climax reports: every non-empty line, delimiter excluded, plays in exact file order during the 365-408 finale window, including every `Software is Supposed to Die` interleave; the final Titanfall line remains locked. Later tracks show their authored track titles instead of these communications.
...
- **Thesis overlay** (`.thesis-overlay` in `src/WolvesApp.vue`): large signal text over the content viewer during the Track 0 finale. Its text constants, mode windows, and corruption glyphs live in `src/data/wolves-thesis-sequence.ts` and are locked authored design. Per explicit owner authorization (2026-07-18) the welcome/support/universal-blue/evolve lines render in the top status bar (`hudLabel`) at their music-synced timestamps (345 to 365 seconds) instead of the center overlay; the center overlay is reserved for the legend cues ("You have ascended ...", "Become Legend", 405 to 425 seconds). After the thesis lines, all incoming-signal messages compress evenly into 365 to 408 seconds, and the Titanfall status holds from 408. Do not re-route text between the layers without a fresh explicit request.
- **Lore viewer** (`src/components/wolves/WolvesLoreColumn.vue`, right column): renders only the current timeline-selected narrative record, driven by `getNarrativeSlotForTime()`; there is no dossier index in this column. Hidden during later-track gallery presentation. During Track 0 on desktop, `TheaterExperience.vue` renders a sidecar below the lore column: a native muted looping 16:9 YouTube documentary playlist iframe (no IFrame Player API, no controls) that does not mount below 1024px. See Lore Records for how entries are organized and added.
...
1. **Top-bar communications**: `src/data/wolves-incoming-signal.txt` via `hudLabel`. During the finale the thesis lines lead the status bar (345 to 365 seconds), the signal messages compress into 365 to 408 seconds, and Titanfall holds from 408.
2. **Thesis story**: text constants in `src/data/wolves-thesis-sequence.ts`. Locked. The center overlay renders only the legend cues; the earlier lines publish through `hudLabel` (owner-authorized 2026-07-18).
...
- **Dossier styling**: the dossier and bulletin views share `src/components/wolves/lore/lore-dossier.scss` (imported once by `WolvesLoreColumn.vue`): `lore-dossier-panel/-header/-eyebrow/-title/-body/-footer`, `lore-spec` (with `--boxed`/`--inline` variants and `lore-spec-key`). Spec keys use lowercase yaml-style grammar (`class:`, `site:`, `bond:`); each fact renders exactly once per panel. Do not style these views with Tailwind utility classes — Tailwind utilities are inert in this build (`@use 'tailwindcss'` through Sass leaves `@tailwind utilities` unprocessed).
...
When the user clicks the primary "Start soundtrack" / "Start Experience" button (the first `idle`/`error` -> playback transition in `WolvesSoundtrack.vue`, or the parent-driven `v-model:playing` transition used by `WolvesApp.vue`'s "JOIN THE EVOLUTION" button), the full-screen `wolves-intro` Destiny segment starts immediately, then hands off directly to the live YouTube playlist.

`wolves-intro` is the official YouTube IFrame Player embed of Bungie's "Destiny 2: Into the Light Cinematic" trailer (default unvoiced `youtubeVideoId: 'BV3BZKbpBns'`, optional voiced override `alternateYoutubeVideoId: 'BKm0TPqeOjY'`) with HTML/CSS character overlay text introducing the six Guardian characters. The shared intro widget exposes the exact `Ikora voice over` checkbox during this segment only, switching sources through object-form `loadVideoById({ videoId, startSeconds })` while preserving native time and pause state. The **#nova4ever** hashtag is an easter egg: during the 48 to 70.5 second montage, three split-second `glitch: true` status cues briefly distort the "fighting for something greater than themselves" top nameplate into **#nova4ever** (RGB-split `wc-nameplate--glitch` treatment in `Nameplate.vue`) before snapping back — it never holds as a standing status. The bottom media plaque reads **The Wolves are Coming**.

During the Comic Hero Shots cue, `https://makemeacomic.com` is presented as a large rounded field-monitor QR display. Its lower subdisplay is the reserved authored-dialogue surface; the current line is `Immortalize a Maintainer`. Keep the imported QR SVG and its quiet zone intact, and keep the complete monitor above the 140px footer on desktop and mobile. The title-card text is **pinned, not flowed**: the `COMIC HERO SHOTS OF OPEN SOURCE MAINTAINERS SHREDDING A BUNCH OF CLANKERS` title and the `Made by Paid Artists` pill are absolutely positioned screen bands on the full-screen card (title near the top, pill above the footer dock) on both desktop and mobile, so the cycling hero art can never shift the text. Do not return the text lines to the flex/grid flow; the layout contract is that text bounding boxes stay pixel-identical across every hero shot. Each `WolvesComicHeroShot` has a measured `contentFrame` derived from its alpha-content bounds; preserve it so transparent canvas padding never controls the visible dinosaur scale. Guardian-bond dinosaurs remain exclusive to their Destiny companion cards and never enter this QR title-card rotation.

`src/data/wolves-comic-hero-shots.ts` registers one high-quality representative for each distinct non-bond visual, determined by rendered image content rather than filenames. Keep its coverage test synchronized when adding an eligible asset. Do not add the guardian-bond assets `alamosaurus`, `bob-torosaurus`, `chonky-alamo-blue`, `chonky-alamo-vector`, `karl`, `header/katharina`, or `torosaurus`; likewise, do not re-add the lower-quality visual duplicates `bluefin-small`, `bluefin-nesting`, `bluefin-pride`, `Jorge_CustomChonks_Intrigued`, `Jorge_CustomChonks_Leaping`, `Jorge_CustomChonks_PivotRaptor_BLMBlackGold_Post`, or `Jorge_CustomChonks_Roaring`.
...
**What agents may touch**: segment data, `youtubeVideoId`/`audioYoutubeVideoId`, `maxDuration`/`duration`, and overlay text/timing/background/highlight/nameplate fields in `buildIntroVideoSequence()` (`src/data/wolves-intro-sequence.ts`), with exact user-supplied copy only — same editorial rule as everywhere else on this page. The Collapse artwork (`public/wolves-intro/bluefin-collapse-{day,night}.webp`) is exclusive to this sequence by explicit user request; do not restore it to the Track 0 wallpaper rotation. It holds as one scene through 45 seconds, then uses the existing scene transition for one fade to black; do not add more Collapse crossfades.
...
- The plate's header label reads **`MAINTAINER // GUARDIAN`** (maintainer first), matching the narrative's own wording ("They were Maintainer-Guardians...", `wolves-intro-sequence.ts`). The lore-column `GuardianDossierView.vue` header must always read the same text — keep both in sync. **Exception:** cues flagged `trustee: true` read **`TRUSTEE // GUARDIAN`** instead (see the trustee bullet below).
- **Trustee plates render in burnished silver** via the `trustee: true` cue field (`wolves-guardian-plate-trustee` CSS in `WolvesIntroOverlay.vue`): silver horizons/crest/label/burst (`#d1d5db`/`#e5e7eb`) and the `TRUSTEE // GUARDIAN` label, distinct from the default blue plate and the singular gold leader plate. Reserved for Universal Blue trustees — Bob Killen uses this intro treatment; Jorge Castro's Ghosts In The Mist theater plate in `WolvesComicReader.vue` mirrors the standard silver palette. Do not apply to other guardians without a fresh user request.
- **Guardian classes are owner-assigned**: Kat Cosgrove is **Sentinel Titan** and Jorge Castro (Ghosts In The Mist plate) is **Harbinger Titan**, swapped by explicit user request; do not revert.
- A guardian's bonded dinosaur renders as a **companion nameplate** — a separate chamfered card anchored independently at the lower-right screen edge (`.wolves-companion-plate` in `WolvesIntroOverlay.vue`), labelled `GUARDIAN BOND`, with the artwork prominent and breaking out over the card's top edge (the card carries the clip-path; the art overlaps via negative bottom margin), plus the dinosaur's authored name (currently Karl, Alamo, and Katerina; Bob's torosaurus remains unnamed and shows species only) and italic scientific name. This applies uniformly to every intro Guardian, including Alamo during Natali Vlatko's raised cue. Guardian plates retain their own anchors and swap transition classes. The intro pre-decodes every companion image at mount, so a late cue never blocks on the first decode; preserve that startup prewarm when adding bonds. The bond lookup lives in `src/data/wolves-guardian-dinosaur-bonds.ts` — a small curated `guardianName -> { dinosaurSpeciesId, dinosaurName? }` list, intentionally *not* parsed from lore markdown at runtime (the intro overlay is a lightweight, performance-sensitive component and shouldn't depend on the full lore loader). Update this list whenever a new `guardian-bond` record is added for a guardian who also appears in the intro sequence, and never invent a dinosaur name — `dinosaurName` must come from an authored character sheet. Guardians with no documented bond render with no companion plate — this is expected, not a bug.
...
- **Christoph Blecker is a leader and trustee**, with `leader: true` and `trustee: true` rendering his **`TRUSTEE // GUARDIAN`** label and complete gold leader plate. The leader treatment takes precedence over the trustee's silver chrome while preserving the trustee classification. His window runs 83-96s, matching Natali Vlatko's end. His title line carries two segments ("First Among Equals — The North Star"), joined and rendered with equal visual weight. **"Uncompromising Purity" and "Platinum Member" (added 2026-07-15, the latter with a `blingTitle: 'Platinum Member'` shimmer) were removed the same day** per a follow-up explicit user request to drop back to just the original two titles — do not re-add either without a fresh user request. `blingTitle` must match a substring of the cue's title exactly (case-sensitive) or it silently renders with no effect.
...
**Once-only rule and content dedupe**: every selected Track 0 shot plays exactly once. The generator collapses byte-identical files within each wallpaper directory to a single manifest entry (`dedupeByContent` in `scripts/generate-wallpapers.js`): the survivor prefers a stem listed in `preferredDuplicateStems` (ids hardcoded in scheduling code), then a curated non-stock filename over a stock feed name (`kubecon-*`, `flickr-*`, `PXL_*`, camera-roll patterns). When adding a captioned copy of an existing stock shot, delete nothing — give the curated copy its title in `curatedTitles` and let the dedupe pick the survivor; never register the same image under two filenames expecting both to play. The pre-legend barrage is a deterministic 30-local-shot music budget; unselected local people carry into the later-track gallery, and CNCF feed photos never backfill the barrage. `src/tests/wolvesComicReader.test.ts` asserts manifest byte-uniqueness, Track 0 uniqueness, and carry-forward behavior; keep all green.

**Note on committing the manifest**: CI regenerates `wallpapers-list.ts` during `npm run build`, so generator changes alone ship to production. Never commit a locally regenerated manifest that references images absent from git — they would 404 in production.

**Weight budget**: wallpapers are slideshow-only and never offered for download, so lossy re-encoding is safe. Keep each file under roughly 500KB: cap the long edge at 2560px, JPEG quality 82 or `cwebp -q 80`, and always re-encode **in place with the exact filename and extension preserved** — the fixed Track 0 slide windows below reference case-sensitive path IDs, and a rename silently breaks locked scheduling. See `docs/skills/wolves-content-maintenance.md` for the full protocol.
...
**Bluefin Microraptor showcase lock**: `wolves/showcase/Bluefin Microraptor by Dr. Natalia Jagielska.webp` is pinned to index 25 of the seeded Track 0 showcase shuffle (owner request), landing it as the fourth slide of the Heavy Chorus 1 stretch. `pinBluefinMicroraptorSlide()` in `src/data/wolves-track-zero-slides.ts` applies the lock after `deterministicShuffle(normalShowcase, 202)` in `WolvesComicReader.vue`, so wallpaper-pool drift never moves the slide. `src/tests/wolvesTrackZeroSlides.test.ts` asserts the lock.

**Measured beat grid (Track 0 re-timing)**: the Track 0 scheduler cuts slides on the measured beat grid in `src/data/wolves-track-zero-beats.ts` (owner-authorized re-timing). The grid holds 1050 librosa-measured beat timestamps from the exact video audio: 152 BPM globally, slowing to ~136 BPM in roughly 172-270s, back at 152 BPM by ~277s; the song rings out to silence by ~419.5s. `TRACK_ZERO_SECTIONS` snaps every section boundary to a measured beat (verse 41.982, chorus 126.851, bridge 229.204, build 276.944, pivotal 344.956, pivotal end 350.482, barrage 359.166, finale hold 408.137), and `trackZeroBeatCuts()` allocates per-slide durations in descending beat tiers so each section opens with long holds and tightens toward its end (intro 32/24 beats, verse 16/8, chorus fill 12/8, post-hero 10/8, bridge 6/4, build 8/4, barrage 8/4/2 across 30 deterministic local shots). The hero locks above take precedence over the grid, the pivotal/bketelsen freezes snap to `pivotalStart`/`pivotalEnd`/`bkEnd` while still covering the 346s/351s thesis anchors, the heart photo insert (index 19 of the build pool) covers the 321s owner anchor, the DN 013 Howl lock (index 0 of the build pool, `kubecon-55177109118.webp`) opens the build-up exactly on the buildStart beat together with the "Falling back to humans/trying-their-best:v1 slowly" status flip at 276.944s, and the finale photo holds from the Become Legend cue through the measured fade to the 423s handoff. Do not hand-edit individual beat values; regenerating the grid requires re-running the audio analysis. `src/tests/wolvesTrackZeroBeats.test.ts` asserts the grid shape and allocator behavior.

**Pacing recovery oracle**: measured beats do not have uniform elapsed durations. For an owner-approved pacing change, assert the leading tier deltas and every later one-beat cut by index in `TRACK_ZERO_BEAT_TIMES`, not through a fixed seconds-per-cut range. Keep `src/tests/wolvesComicReader.test.ts` aligned with the final-image start, and make `tests/wolves-movie-flow.mjs` seek the final boundary to assert the visible handoff.

**Later-track shuffle**: after the Track 0 hero presentation, Track 1 ("Ghosts In The Mist") opens with the owner-authored `MN_047` Jorge photo and quote registered in `src/data/wolves-gallery-featured.ts`. The plate renders in the burnished silver trustee treatment with the label **TRUSTEE // GUARDIAN**, class **Harbinger Titan**, and the title separator (`|`) in searing blue (`#38bdf8` with a layered glow). The quote is authored as `descriptionParts` — two halves that play sequentially across the 38.4-second hold (switching at the midpoint) so the plate stays compact; never render all paragraphs at once. After the hold it hands off to one complete Fisher-Yates shuffle of every remaining available photo across songs 2 onward. The featured opener is removed from the shuffled pool, so it cannot repeat. The gallery does not otherwise group, rotate, reuse, or order photos by title. Once the available pool is exhausted, the final image holds rather than restarting the sequence. This does not change Track 0's authored slideshow or its timing locks. `src/data/wolves-gallery-shuffle.ts` owns the shuffle behavior.
...
| `src/data/wolves-incoming-signal.txt` | Top-bar communication lines (one message per line). The first line is the only one shown before the hero run (through 175.96); the rest of the pre-`---` ambient signals play 196.36-229, after the heroes they reference. The climax reports below the `---` only appear in the 365-408 finale compression window. Affects the top status bar only; never a substitute for thesis or lore text |
...
10. For Guardian plate recovery, retain the current Destiny source-switching and pause/resume tests, and assert the documented dinosaur companion nameplate (`GUARDIAN BOND` card beside the guardian plate, owner-requested 2026-07-16; it replaced the earlier small in-name icon). Do not replace the current overlay test with a historical snapshot.
