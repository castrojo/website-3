...
- **Track 0 nameplate communications** (`.wc-stage-nameplate`): the large label text comes from `src/data/wolves-incoming-signal.txt` (see Open Surfaces) and changes with a 1.5-second fade. The plate (`Nameplate.vue`, shared by the intro and stage) carries a Bluefin-branded emblem on its left: a small gold-ringed roundel that opens on the Bluefin raptor avatar and crossfades through the full `dinosaurSpecies` roster every 20 seconds (decorative, `aria-hidden`). The default HUD label is `Celebrating Five Years of Universal Blue`. Only the first line (currently `Welcome to Indie Cloud Native`) holds from load through 175.96 seconds — the remaining ambient signals name heroes (Hikari, M2, Mechaphippy) and would spoil the story if shown before their slides. The label holds `The Blue Delivers` for the locked Bluefin group through 196.36 seconds, then the remaining pre-delimiter ambient signals compress evenly into 196.36 to 229 seconds (right after the heroes they reference, ending on the pod line), followed by the split ImagePullBackOff and human-fallback lines through 345 seconds. A `---` line in the source splits those ambient signals from the climax reports: the full list (climax reports included, delimiter dropped) compresses into the 365-408 finale window, then the final Titanfall line remains locked. Later tracks show their authored track titles instead of these communications.
...
- **Trustee plates render in burnished silver** via the `trustee: true` cue field (`wolves-guardian-plate-trustee` CSS in `WolvesIntroOverlay.vue`): silver horizons/crest/label/burst (`#d1d5db`/`#e5e7eb`) and the `TRUSTEE // GUARDIAN` label, distinct from both the default blue plate and the gold leader treatment. Reserved for Universal Blue trustees — currently Bob Killen and Christoph Blecker's intro cues; Jorge Castro's Ghosts In The Mist theater plate in `WolvesComicReader.vue` mirrors the same silver palette. Do not apply to other guardians without a fresh user request.
...
- A guardian's bonded dinosaur renders as a **companion nameplate** — a separate chamfered card anchored independently at the lower-right screen edge (`.wolves-companion-plate` in `WolvesIntroOverlay.vue`), labelled `GUARDIAN BOND`, with the artwork prominent and breaking out over the card's top edge (the card carries the clip-path; the art overlaps via negative bottom margin), plus the dinosaur's authored name (only if a lore character sheet names it — currently Karl and Alamo; Bob's and Kaslin's torosauruses are unnamed and show species only) and italic scientific name. This applies uniformly to every intro Guardian, including Alamo during Natali Vlatko's raised cue. Guardian plates retain their own anchors and swap transition classes. The bond lookup lives in `src/data/wolves-guardian-dinosaur-bonds.ts` — a small curated `guardianName -> { dinosaurSpeciesId, dinosaurName? }` list, intentionally *not* parsed from lore markdown at runtime (the intro overlay is a lightweight, performance-sensitive component and shouldn't depend on the full lore loader). Update this list whenever a new `guardian-bond` record is added for a guardian who also appears in the intro sequence, and never invent a dinosaur name — `dinosaurName` must come from an authored character sheet. Guardians with no documented bond render with no companion plate — this is expected, not a bug.
...
**Once-only rule and content dedupe**: every shot plays exactly once in the Track 0 slideshow. The generator collapses byte-identical files within each wallpaper directory to a single manifest entry (`dedupeByContent` in `scripts/generate-wallpapers.js`): the survivor prefers a stem listed in `preferredDuplicateStems` (ids hardcoded in scheduling code), then a curated non-stock filename over a stock feed name (`kubecon-*`, `flickr-*`, `PXL_*`, camera-roll patterns). When adding a captioned copy of an existing stock shot, delete nothing — give the curated copy its title in `curatedTitles` and let the dedupe pick the survivor; never register the same image under two filenames expecting both to play. When the local pool falls short of the finale beat-barrage budget, `WolvesComicReader.vue` tops up with reserved CNCF feed photos (deterministic shuffle, seed 505, up to 24, only when the eligible feed is at least three times the reservation) and holds those ids out of the later-track rotations so nothing repeats across tracks. `src/tests/wolvesComicReader.test.ts` asserts manifest byte-uniqueness and once-only playback; keep both green.
...
**Measured beat grid (Track 0 re-timing)**: the Track 0 scheduler cuts slides on the measured beat grid in `src/data/wolves-track-zero-beats.ts` (owner-authorized re-timing). The grid holds 1050 librosa-measured beat timestamps from the exact video audio: 152 BPM globally, slowing to ~136 BPM in roughly 172-270s, back at 152 BPM by ~277s; the last barrage accent lands at ~405.7s and the song rings out to silence by ~419.5s. `TRACK_ZERO_SECTIONS` snaps every section boundary to a measured beat (verse 41.982, chorus 126.851, bridge 229.204, build 276.944, pivotal 344.956, pivotal end 350.482, barrage 359.166, finale hold 405.745), and `trackZeroBeatCuts()` allocates per-slide durations in descending beat tiers so each section opens with long holds and tightens toward its end (intro 32/24 beats, verse 16/8, chorus fill 12/8, post-hero 10/8, bridge 6/4, build 8/4, barrage 2/1 — one cut per beat). The hero locks above take precedence over the grid, the pivotal/bketelsen freezes snap to `pivotalStart`/`pivotalEnd`/`bkEnd` while still covering the 346s/351s thesis anchors, the heart photo insert (index 19 of the build pool) covers the 321s owner anchor, the DN 013 Howl lock (index 0 of the build pool, `kubecon-55177109118.webp`) opens the build-up exactly on the buildStart beat together with the "Falling back to humans/trying-their-best:v1 slowly" status flip at 276.944s, and the finale photo holds from the last accent through the measured fade to the 423s handoff. Do not hand-edit individual beat values; regenerating the grid requires re-running the audio analysis. `src/tests/wolvesTrackZeroBeats.test.ts` asserts the grid shape and allocator behavior.
