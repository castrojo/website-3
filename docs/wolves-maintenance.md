...
- **The default unvoiced `wolves-intro` source's existing `maxDuration: 121.5` black-frame cutoff is a separate visual cutoff, retained unchanged**, based on real-player verification that its black-frame outro begins at ~119.0s. This is independent from the local caption track, which is timed to the voiced `BKm0TPqeOjY` transcript and whose restored full dialogue ends explicitly at 112.60s. The optional Ikora-voiced override clamps to its own shorter end (`alternateMaxDuration: 120.2`), also unchanged. Do not move either value without re-verifying the exact black-frame boundary on the real player.
