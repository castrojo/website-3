...
- **Track 0 nameplate communications** (`.wc-stage-nameplate`): the large label text comes from `src/data/wolves-incoming-signal.txt` (see Open Surfaces) and changes with a 1.5-second fade. The source rotates on 16-beat phrases through 175.96 seconds, holds `The Blue Delivers` for the locked Bluefin group through 196.36 seconds, then shows the pod, split ImagePullBackOff, and human-fallback lines through 345 seconds. The source resumes one message per phrase from 345–408 seconds, then the final Titanfall line remains locked. Later tracks show their authored track titles instead of these communications.
...
10. For Guardian plate recovery, retain the current Destiny source-switching and pause/resume tests, and assert the documented small bonded-dinosaur icon. Do not replace the current overlay test with a historical snapshot or turn the icon into a panel.
