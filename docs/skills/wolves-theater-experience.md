...
19. **Timeline-Driven Experimental Slideshow (Track 0):** Guarded by `isExperimental = true`. Precomputes a flat chronological timeline mapping playhead time directly to local showcase/people slides and CNCF Flickr photos to avoid repeats. Day/night slides get double-length duration and fade smoothly from Day (opacity 0) to Night (opacity 1) over their timeline window.
