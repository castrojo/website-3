...
15. **Rate-Limit-Safe CNCF/Flickr Ingestion:** The Flickr harvester (\`scripts/update-flickr-photos.js\`) increments the existing cache with at most 100 unique IDs. Query \`flickr.photos.search\` for \`Maintainer\` and \`flickr.people.getPublicPhotos\` sequentially with a 250ms gap; on HTTP 429, respect \`Retry-After\` and retry with bounded exponential backoff. Deduplicate by Flickr ID before shuffling and writing \`public/flickr-photos.json\`. Source: \`/flickr/flickr-sdk\`.
...
17. **Later-Track Flickr Snapshot:** At the start of each later track, snapshot one Flickr-only CNCF image sequence and retain it for that song. While the cache is loading or unavailable, render no gallery photo; never fall back to local imagery. New feed photos and playlist tracks join automatically without per-track configuration.
...
19. **Timeline-Driven Beat-Matched Slideshow (Track 0):** Guarded by \`isExperimental = true\`. Parses the 423-second first song into a 6-section timeline (Intro, Verse 1, Verse 2/Chorus, Bridge, Build-Up, and Climax) to schedule every generated local wallpaper exactly once without duplicates, using a pure deterministic sine-based PRNG shuffle. Locked images must be inserted into their timeline pools rather than replacing a unique image. Slide transitions dynamically adjust their duration (e.g., 30% of slide length) to keep up with the fast-paced climax (down to 1.18s per slide).
20. **Exclusive, Non-Repeating Flickr Stream for Later-Track Gallery Mode:** Shuffle the Flickr pool once per playback session, then rotate each of the six later tracks to its own 100-image segment. Retain each sequence for its song and render only CNCF Flickr images. This prevents songs 02–07 from reusing the same early images.
...
22. **Continuous Timeline Boundary Matching:** When calculating the active slide for a continuous timeline (where Slide A ends exactly when Slide B begins), avoid boundary checks like \`find(s => time >= s.startTime && time < s.endTime)\`. Resolve with \`findIndex(s => time < s.endTime - 0.001)\` so accumulated floating-point error cannot leave exact timestamps such as 5:19 on the preceding slide.
...
28. **Track 0 Narrative Timeline:** `wolvesNarrativeTimeline` is the sole narrative clock. It presents every `wolvesRelease.artifacts` entry exactly once across Track 0. The first entry begins at `0:00`, `lorem-pursuit-1` begins at `2:30` and holds through `3:40`, and `blue-universal-acquires-wayland-yutani` begins at `6:38` and holds through `7:05`. All non-anchor entries are ordered by `publishedAt`, retaining manifest order only when dates match, then weighted by artifact text length within the free ranges. Schedule with `body.trim().length` to preserve legacy weight boundaries, while rendering the raw `body` byte-for-byte. Lore front matter supplies recovered artifacts' channel and date; never rewrite the source body to change placement. Parse it with `load(input: string): unknown` from `js-yaml`, then verify the result is a mapping before reading fields.
29. **Universal Blue Thesis Sequence:** Preserve the 5:45–7:05 call-to-action independently from lore. It uses `Incoming Signal: Universal Blue`, two equal Jorge-slide callouts from 5:45–5:50.5, `We are Universal Blue.`, `Evolve or die.`, growing corruption, then `You have ascended.` and the foreground `Become Legend` banner. Keep `truly a great loss for humanity.` intact in the final lore artifact and render it as a slow, fading lore-column warning from 6:45–7:05.
30. **Editable Incoming Signal Phrases:** Keep the signal content in `src/data/wolves-incoming-signal.txt`, with one non-empty line per message. Import it with Vite's `?raw` asset query, trim and discard empty lines, then select a phrase using `floor((time - 345) * 152 / 60 / 16)`, capped at the final message. The first line begins at 5:45, each phrase lasts sixteen beats, and the final deployment line holds through 7:05. A blank file renders no substitute copy. Keep the thesis active through 7:05 so the HUD and the existing day-pulse, corruption, and legend modes remain visible. Source: `/vitejs/vite`.
30. **Read-Only Graphic Novel and Playlist Navigation:** Comic and lore controls must not paginate, rotate, or change a Track 0 narrative position. Track navigation may use only the YouTube IFrame Player API's native `previousVideo()` and `nextVideo()` methods. Disable Previous at playlist index zero and Next at the final index: without looping, the native API restarts the first track or ends playback at those boundaries. The typewriter scales to each Track 0 timeline slot; later tracks do not render narrative lore. Render only source characters, use a 5ms–50ms step interval, and never auto-scroll on each character tick. Source: `/websites/developers_google_youtube`.
