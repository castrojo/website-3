...
28. **Chapter-Specific Lore Filtering:** Active quotes and dialogues are mapped 1-to-1 to active pages via `pageToArtifactIdMap` in `WolvesLoreColumn.vue`. Unifying or dumping lore entries into a single list or rotating them dynamically under track-driven mode is disabled. Always preserve the exact page-to-artifact IDs mapping.
...
30. **Master Playhead Narrative Drive & Non-Repeating Lore:** The soundtrack master playhead (Track 0) drives the active page number (1 to 20) exactly, mapping to precise hardcoded timeline segments. This prevents micro-gaps, skipping, or repeats. Every page and quote is presented exactly once chronologically, like pages in a book. Dynamic auto-rotation and slider navigation (including `startLoreTimer`, `nextLore`, and `prevLore` in non-track-driven modes) are strictly non-wrapping: once the last item is reached, it locks and halts in place, and never wraps around or restarts from the beginning.
31. **Strict Playlist Manifest Generation:** The Wolves soundtrack playlist manifest (`public/wolves-playlist.json`) is strictly generated by `scripts/update-wolves-playlist.js`. It enforces precise schema validation: YouTube playlist items fetched via `yt-dlp` must possess non-empty string properties for `id`, `title`, and `uploader`. Thumbnails must resolve to a valid `.jpg` URL on YouTube's servers or safely fall back. Downloaded artworks are saved precisely under `public/wolves-artwork/{id}.jpg`. Each track in the manifest must automatically merge with matching `TEMPO_CONFIGS` (BPM, phrase beats, and fade duration constants) to ensure perfect timing calculations at runtime, outputting clean, indented JSON with a trailing newline. Do not edit `wolves-playlist.json` manually; always run the updater script to rebuild it.
